...
const messages = defineMessages({
  sellOrderConfirmMessageMsg: {
    id: 'orderConfirm.message',
    defaultMessage: 'You are about to {txDesc}. Please click OK to continue.',
  },
  sellOrderConfirmMessageWithFeeMsg: {
    id: 'orderConfirm.messageWithFee',
    defaultMessage: 'You are about to {txDesc} with a maximum transaction fee of {txFee} RUNES. Any unused transaction fees will be refunded to you. Please click OK to continue.',
  },
  strTypeMsg: {
    id: 'str.type',
    defaultMessage: 'Type',
...
export default class SellOrderConfirmDialog extends Component {
  render() {
    ...
    let confirmMessage = formatMessage(messages.sellOrderConfirmMessageMsg, { txDesc, txAmount, txToken, txPrice, txTotal });
    if (txFee) {
      confirmMessage = formatMessage(messages.sellOrderConfirmMessageWithFeeMsg, { txDesc, txAmount, txToken, txFee, txPrice, txTotal });
    }
    if (!open) {
      return null;
    }
    ...
    return (
      <Dialog maxWidth={false} open={open}>
        <DialogTitle>
          <FormattedMessage id="sellOrderConfirm.title" defaultMessage="Please Confirm Your Sell Order" />
        </DialogTitle>
        <DialogContent>
          {confirmMessage}
